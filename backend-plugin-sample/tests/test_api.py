...
def test_list_course_archive_status_authenticated(
    ...
    url = reverse("openedx_plugin_sample:course-archive-status-list")
...
def test_update_course_archive_status(api_client, user, course_archive_status):
    ...
    api_client.force_authenticate(user=user)
    url = reverse(
        "openedx_plugin_sample:course-archive-status-detail", args=[course_archive_status.id]
    )
...
def test_staff_update_with_explicit_user_override(
    ...
    api_client.force_authenticate(user=staff_user)
    url = reverse(
        "openedx_plugin_sample:course-archive-status-detail", args=[initial_status.id]
    )
    data = {
        "user": another_user.id,
        "is_archived": True,
